...
const Footer = () => {
 ...
 const MailIcon = () => (<svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg"> <path d="M14.6654 4.66699L8.67136 8.48499C8.46796 8.60313 8.23692 8.66536 8.0017 8.66536C7.76647 8.66536 7.53544 8.60313 7.33203 8.48499L1.33203 4.66699M2.66536 2.66699H13.332C14.0684 2.66699 14.6654 3.26395 14.6654 4.00033V12.0003C14.6654 12.7367 14.0684 13.3337 13.332 13.3337H2.66536C1.92898 13.3337 1.33203 12.7367 1.33203 12.0003V4.00033C1.33203 3.26395 1.92898 2.66699 2.66536 2.66699Z" stroke="#90A1B9" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /> </svg>)
  ...
  ======================= */
  return (
    <footer className="bg-[#fffdf4] px-6">
      <div className="max-w-7xl mx-auto">

        <div className="flex flex-col md:flex-row justify-between gap-12 py-14 border-b border-black/10 text-black">
          ...
          <div>
            ...
            <p className="max-w-[420px] mt-6 text-sm leading-relaxed">
              Welcome to <strong>AL ZAL STREET FOOD</strong> — the heart of authentic flavors.
              From shawarma and burgers to fresh juices and desserts, we serve
              the real taste of the street with quality and passion.
            </p>
            ...
            <div className="flex items-center gap-4 mt-6">
              {socialIcons.map((item, i) => (
                <Link
                  key={i}
                  href={item.link}
                  className="w-10 h-10 flex items-center justify-center rounded-full border border-black/10 hover:border-[#6b1415] hover:text-[#6b1415] transition"
                >
                  <item.icon />
                </Link>
              ))}
          ...
          <div className="flex flex-wrap justify-between w-full md:w-[45%] gap-8 text-sm">
            {linkSections.map((section, index) => (
              <div key={index}>
                <h3 className="text-[#6b1415] font-semibold mb-4">
                  {section.title}
                </h3>
                ...
                <ul className="space-y-2">
                  {section.links.map((link, i) => (
                    <li key={i} className="flex items-center gap-2">
                      {link.icon && <link.icon />}
                      <Link href={link.path} className="hover:text-[#6b1415] transition">
                        {link.text}
                      </Link>
                    </li>
                  ))}
        ...
        <div className="py-4 text-sm text-black flex flex-col md:flex-row justify-between gap-2">
          <p>© 2025 AL ZAL STREET FOOD. All rights reserved.</p>
